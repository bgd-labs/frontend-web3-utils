...
class GelatoAdapter {
  constructor(get, set) {
    ...
    this.updateGelatoTX = (taskId, statusResponse) => {
      this.set(state => produce(state, draft => {
...
var useLastTxLocalStatus = _ref => {
  ...
  var [fullTxErrorMessage, setFullTxErrorMessage] = useState('');
  var [error, setError] = useState('');
  var [loading, setLoading] = useState(false);
  var [isTxStart, setIsTxStart] = useState(false);
    ...
    return _asyncToGenerator(function* () {
      ...
      if (network == null) {
        ...
        if (_this._network == null) {
          ...
          defineReadOnly(_this, '_network', network);
...
class ImpersonatedProvider extends providers.JsonRpcProvider {
  constructor(url) {
    super(url);
...
class ImpersonatedConnector extends Connector {
...
var initAllConnectors = props => {
  ...
  var metaMask = initializeConnector(actions => new MetaMask({
    actions
  }));
  var walletConnect = null;

  if (projectId) {
    walletConnect = initializeConnector(actions => new WalletConnect({
      actions,
      options: {
        projectId,
        chains: chainIds,
  ...
  var coinbase = initializeConnector(actions => new CoinbaseWallet({
    actions,
    options: {
  ...
  var gnosisSafe = initializeConnector(actions => new GnosisSafe({
    actions
  }));
  var impersonatedConnector = initializeConnector(actions => new ImpersonatedConnector(actions, {
    urls: props.urls,
...
function Child(_ref) {
  ...
  var [currentWalletType, setCurrentWalletType] = useState('');
  useEffect(() => {
    if (connectors) {
...
function Web3Provider(_ref2) {
  ...
  var [connectors] = useState(initAllConnectors(connectorsInitProps));
  var [mappedConnectors] = useState(connectors.map(connector => connector[0]));
  ...
  return (set, get) => ({
    ...
    initDefaultWallet: function () {
      var _initDefaultWallet = _asyncToGenerator(function* () {
        var lastConnectedWallet = localStorage.getItem(LocalStorageKeys.LastConnectedWallet);
